...
# v0.85b Rott 1.0.7 compatible, replace user 'pi' with 'tinker'
    ...
    case "$CASE_SEL" in
        ...
        "--ES-REBOOT")
            ...
            if [[ -n $ES_PID ]]; then
                ...
                chown tinker:tinker /tmp/es-sysrestart
                kill $ES_PID
                wait_forpid $ES_PID
                exit
            fi
        ...
        "--ES-POWEROFF")
            ...
            if [[ -n $ES_PID ]]; then
                ...
                chown tinker:tinker /tmp/es-shutdown
                kill $ES_PID
                wait_forpid $ES_PID
                exit
            fi
        ...
        "--ES-RESTART")
            ...
            if [[ -n $ES_PID ]]; then
                ...
                chown tinker:tinker /tmp/es-restart
                kill $ES_PID
                wait_forpid $ES_PID
            fi
        ;;
